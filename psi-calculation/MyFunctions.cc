...
float TruthPsiHadHadBump(
    ...
    TLorentzVector beam;
    // beam.SetPxPyPzE(0., 0., 6.5e6, 6.5e6);
    beam.SetPxPyPzE(bump_px, bump_py, 6.5e6+bump_pz, 6.5e6);
...
float TruthPsiHadHadTrueBoostRhoPlane(
    ...
    // Boost piPlus and piMinus
    rhoPlus_tlv.Boost(my_boost);
    rhoMinus_tlv.Boost(my_boost);

    // Rotate beam and piPlus with angles of piMinus
    beam.RotateZ((-1.0)*rhoMinus_tlv.Phi());
    beam.RotateY((-1.0)*rhoMinus_tlv.Theta());
    rhoPlus_tlv.RotateZ((-1.0)*rhoMinus_tlv.Phi());
    rhoPlus_tlv.RotateY((-1.0)*rhoMinus_tlv.Theta());

    // Rotate beam with angle of piPlus
    beam.RotateZ((-1.0)*rhoPlus_tlv.Phi());
...
float Upsilon(
    ...
    const double upsilon = tau_char_pion.E()/(tau_char_pion.E()+tau_neut_pion.E());
...
float UpsilonHiggs(
    ...
    const double upsilon = (tau_char_pion.E()-tau_neut_pion.E())/(tau_char_pion.E()+tau_neut_pion.E());
